...
| A-050 | Polymarket maker rewards parameters (min size/spread bands, daily pools) are readable per-market. Verified: `rewardsMinSize`/`rewardsMaxSpread` on Gamma AND full configs incl. `rate_per_day` on CLOB `/rewards/markets/*`; consumed by `GET /api/markets/:conditionId/economics` and the MakerEstimator (INTEGRATION_VERIFIED §19). | RESOLVED (verified 2026-07-15) |

## Round 6 — pUSD, withdrawals, live farming (D-026)

| ID    | Assumption                                                                                                                                                   | Status / verification                                                                                                                                        |
| ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| A-051 | USDC.e sent to a deposit wallet is auto-converted to pUSD by Polymarket's infrastructure (deposits remain the documented top-up path).                       | OPEN — owner staging test §2 of OWNER_TESTING_GUIDE verifies with $5; the Funds sheet surfaces unconverted USDC.e as "converting…" so a failure is visible.  |
| A-052 | pUSD received in an external EOA is redeemable 1:1 (via Polymarket deposit/redeem); withdrawals therefore deliver full value.                                | PARTIAL — on-chain simulation confirms transferability to the owner EOA (INTEGRATION §23); redemption UX is Polymarket-side, owner verifies once at staging. |
| A-053 | Privy's signing policy permits the ERC-7739 TypedDataSign envelope (primaryType `TypedDataSign`, domain = the deposit wallet) that POLY_1271 orders require. | OPEN — mock signer accepts anything; first real order at staging (OWNER_TESTING_GUIDE §3c) proves it. Fallback: extend the Privy policy allowlist.           |
| A-054 | The CLOB `getEarningsForUserForDay` endpoint reports liquidity-rewards accrual per conditionId for the deposit-wallet identity's L2 creds.                   | OPEN — SDK method exists and is wired (rewards poller); numbers verified against the Polymarket UI during the owner's confirm-mode trial.                    |
