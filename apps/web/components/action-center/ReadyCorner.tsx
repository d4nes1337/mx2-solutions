 ...
 * still lives in the bell). AUTO_EXECUTED receipts (W5) render here with the
 * same weight: they stay until dismissed — Dismiss acknowledges server-side,
 * so the receipt clears on every device. Renders regardless of the
 * browser-alerts opt-in — this is in-app UI, not an OS notification.
 */
import { X } from "lucide-react";
import { Badge, Button } from "@/components/ui";
import { useAckTrigger } from "@/lib/queries";
        ...
        return (
          ...
          >
            <div className="flex items-start justify-between gap-2">
              <Badge tone={executed ? "pos" : "brand"} dot>
                {executed ? "Executed automatically" : "Conditions met"}
              </Badge>
              <button
                type="button"
                aria-label={executed ? "Dismiss" : "Not now"}
                onClick={() => (executed ? ack.mutate(item.triggerId) : onLater(item.triggerId))}
            ...
            <p className="text-[12px] text-muted">
              ...
              {item.market.outcome} at {executed ? "" : "up to "}
              {cents(item.action.price)} · max ${item.action.maxSpendUsd}
            </p>
            <div className="mt-2 flex items-center gap-2">
              <Button size="sm" onClick={() => onReview(item.triggerId)}>
                {executed ? "View details" : "Review & sign"}
              </Button>
              <button
                type="button"
                disabled={executed && ack.isPending}
                onClick={() => (executed ? ack.mutate(item.triggerId) : onLater(item.triggerId))}
                className="text-[12px] text-muted transition-colors hover:text-fg disabled:opacity-50"
              >
                {executed ? "Dismiss" : "Not now"}
              </button>
            </div>
          </div>
        );
